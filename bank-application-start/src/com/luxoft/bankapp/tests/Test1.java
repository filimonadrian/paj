...
public class Test1 {
	...
	public void testBank() throws ClientExistsException {
		...
		Client client2 = new Client("Smith Michelle", Gender.FEMALE, "Constanta");
		...
		assertTrue("Check if first client exists in the Set", bank.getClients().contains(client1));
		assertTrue("Check if second client exists in the Set", bank.getClients().contains(client1));

		for(Client client : bank.getClients()) {
			if (client.getName().equals(client1.getName())) {
				assertEquals(client1.hashCode(), client.hashCode());
				assertEquals("Mr. Smith John", client.getClientGreeting());
				assertEquals("Mr. Smith John", client.toString());
			} else {
